...
from mrstarkfunction import check_if_subbed
...
mrstarkbot = bot.start(bot_token=Config.BOT_TOKEN)
...
@mrstarkbot.on(events.NewMessage(pattern="^/start$"))
async def hmm(event):
    if Config.JTU_ENABLE:
    	starky = await check_if_subbed(Config.CHANNEL_USERNAME, event, mrstarkbot)
    	if starky is False:
...
Bot Made With ❤️ By @MrStarkXD**
""" 
    await event.reply(user_text) 
    
@mrstarkbot.on(events.NewMessage(pattern="^/(help|cmds|commands|cmd|command)$"))
...
@mrstarkbot.on(events.NewMessage(pattern="^/(generate|gen|account)$"))
async def hmm(event):
    if Config.JTU_ENABLE:
    	starky = await check_if_subbed(Config.CHANNEL_USERNAME, event, mrstarkbot)
    	if starky is False:
    ...
    user_s = await mrstarkbot.get_me()
...
@mrstarkbot.on(events.NewMessage(pattern="^/reset$"))
...
@mrstarkbot.on(events.NewMessage(pattern="^/broadcast"))
async def reset(event):
    if event.sender_id != Config.OWNER_ID:
        return
    error = 0
    ...
    for s in ok:
        try:
            await mrstarkbot.send_message(int(s['user']), ds)
        except:
            error += 1
            pass
...
async def clear_data():
    ...
    for s in ok:
        try:
            await mrstarkbot.send_message(int(s['user']), "**Limit Has Been Reset , Generate Your Accounts Now !**")
        except:
            error += 1
            pass
    dl_all_users()
        
@mrstarkbot.on(events.NewMessage(pattern="^/about$"))
async def a(event):
...
def startbot():
    mrstarkbot.run_until_disconnected()
